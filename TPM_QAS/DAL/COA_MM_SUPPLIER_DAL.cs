...
{
    public class COA_MM_SUPPLIER_DAL : Database
    {
        public async Task<DataTable> getSupplier_Data(string ID, string type)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_SUPPLIER_SEL", con))
                    {
                        await con.OpenAsync();
        ...
        public async Task<string> Supplier_H_Maint(COASupplierModel model)
        {
            string result = "0";
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_SUPPLIER_H_MAINT", con))
                    {
                        await con.OpenAsync();
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;

                        cmd.Parameters.Add(new SqlParameter("@pID", model.MM_SUPPLIER_H_ID)).Direction = ParameterDirection.Input;
                        cmd.Parameters.Add(new SqlParameter("@pName", model.SUPPLIER_NAME)).Direction = ParameterDirection.Input;
        ...
        public async Task<string> Supplier_D_Maint(int pid, int phid, string keytype, string suppkey, string rectype)
        {
            string result = "0";
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_SUPPLIER_D_MAINT", con))
                    {
                        await con.OpenAsync();
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;
                        ...
                        cmd.Parameters.Add(new SqlParameter("@pKeyType", keytype)).Direction = ParameterDirection.Input; 
                        cmd.Parameters.Add(new SqlParameter("@pSuppKey", suppkey)).Direction = ParameterDirection.Input;
