...
{
    public class MM_COMMON_GROUP_DAL : Database
    {
        public async Task<DataTable> getCommonGroup_Data(string ID, string type)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_COMMONGROUP_SEL", con))
                    {
                        await con.OpenAsync();
        ...
        public async Task<DataTable> getPropertyColorPOPUP(string type)
        {
            ACL_UserObj userobj = HttpContextHelper.Current.Session.GetObject<ACL_UserObj>("AclUser");
            string USERID = userobj.EMP_NAME.ToString();
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_COMMONGROUP_COLOR_POPUP", con))
                    {
                        await con.OpenAsync();
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;
        ...
        public async Task<string> CommonGroup_H_Maint(CommonGroupModel model)
        {
            string result = "0";
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_COMMONGROUP_H_MAINT", con))
                    {
                        await con.OpenAsync();
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;

                        cmd.Parameters.Add(new SqlParameter("@pID", model.MM_COMMONGROUP_H_ID)).Direction = ParameterDirection.Input;
                        cmd.Parameters.Add(new SqlParameter("@pCategory", model.CATEGORY)).Direction = ParameterDirection.Input;
        ...
        public async Task<string> CommonGroup_D_Maint(int pid, int phid, string subcategory, string items, string rectype)
        {
            string result = "0";
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("PSP_MM_COMMONGROUP_D_MAINT", con))
                    {
                        await con.OpenAsync();
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;
                        ...
                        cmd.Parameters.Add(new SqlParameter("@pSubCategory", subcategory)).Direction = ParameterDirection.Input;
                        cmd.Parameters.Add(new SqlParameter("@pItems", items)).Direction = ParameterDirection.Input;
